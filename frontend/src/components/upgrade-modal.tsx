...
export function UpgradeModal({ error, onClose }: { error: ApiError | null; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        style={{ background: '#fff', borderRadius: 14, padding: 28, maxWidth: 380, width: '90%' }}
        onClick={(e) => e.stopPropagation()}
      >
        <div style={{ fontSize: 13, color: '#a3552d', fontWeight: 600, marginBottom: 8 }}>
          Plafond atteint
        </div>
        <h2 style={{ fontSize: 18, margin: '0 0 8px' }}>Vous avez utilisé {limitLabel}</h2>
        <p style={{ fontSize: 13.5, color: '#5f5e5a', lineHeight: 1.6, marginBottom: 20 }}>
        ...
        {recommended && (
          <div style={{ border: '1px solid #e5e3dd', borderRadius: 10, padding: 16, marginBottom: 20 }}>
            ...
            <span style={{ fontSize: 12.5, color: '#4a9d7f' }}>
