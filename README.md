...
trompeuse. `GoogleAdsAdapter.fetchInsights()` remonte les conversions réelles côté lecture
(tracking déjà configuré côté compte client) ; Meta et LinkedIn ne le font pas (organique,
pas d'API de conversions branchée). Les conversions saisies manuellement sont désormais aussi
poussées vers Meta Conversions API quand un pixel est configuré et qu'un clic récent existe
pour l'attribution (cf. item 105, `MetaCapiService`) — l'endpoint manuel reste la voie
d'entrée dans tous les cas, ce push est un enrichissement, pas un remplacement.
...
29. ~~Intégration Conversions API réelle~~ ✅ **partiellement fait (Meta uniquement)** — cf. item 105. Google Ads reste hors scope (bloqué sans `gclid` réel du compte publicitaire client, contrainte dure de son API, pas un réglage de qualité).
...
104. ~~Couverture e2e Playwright limitée à l'inscription/onboarding/campagne et à l'i18n~~ ✅ étendue — 2 nouvelles specs contre la stack réelle (Postgres+Redis+backend+frontend, `AI_MODE=mock`) : `team.spec.ts` (inviter un membre → apparaît dans les invitations en attente → renvoyer → annuler retire la ligne — n'exerce pas l'acceptation d'invitation, dont la page frontend est absente, cf. item 103) et `billing.spec.ts` (quota d'essai réel `0/300`, grille des 4 plans payants avec leurs vraies valeurs `plan-catalog.ts` — prix, crédits, sièges, campagnes, canaux — sans jamais cliquer "Choose this plan"/"Contact us", ce qui déclencherait un vrai appel Stripe non configuré en CI). Assertions par texte volontairement non scopées par carte DOM (une tentative avec `page.locator('div', {has: ...})` s'est révélée ambiguë — un `div` "a" pour descendant aussi bien la carte que son conteneur grille — remplacée par des valeurs numériques uniques par plan et un comptage (`toHaveCount`) pour les deux libellés partagés entre cartes). Deux ambiguïtés similaires corrigées avant même le premier run CI : le lien de navigation "Team" duplique le texte du titre de page (`getByRole('heading', ...)` plutôt que `getByText`), et "EDITOR" existe à la fois comme option du sélecteur de rôle et comme contenu de la ligne d'invitation.
105. ~~Intégration Conversions API réelle (item 29)~~ ✅ **Meta uniquement** — recherche préalable (2 agents d'exploration, code lu en entier) : ni Meta CAPI ni Google Ads offline conversion upload ne pouvaient fonctionner, ce dépôt ne traçant aucun identifiant de clic (`fbclid`/`gclid`, zéro résultat en grep exhaustif avant ce chantier). `GoogleAdsAdapter.fetchInsights()` remonte déjà les conversions réelles côté LECTURE (GAQL, tracking déjà configuré côté client) — ce n'était jamais le vrai gap ; `uploadClickConversions` (écriture) reste bloqué sans `gclid` réel, exclu explicitement (décision produit). Construit pour Meta : (1) `PublishedPost.destinationUrl` — l'URL réelle du client, jusque-là purement transitoire (`linkUrl` de `PublishTargetDto`), désormais persistée au moment de la publication ; (2) `GET /r/:publishedPostId` (public, `ClickTrackingController`) — l'URL réellement transmise à la plateforme à la place de l'URL client, capture `fbclid`/`gclid` de la query string dans un nouveau modèle `TrackedClick` avant de rediriger vers la vraie destination ; (3) `MetaCapiConfig` (par ORGANISATION, jamais une variable d'environnement globale — chaque client a son propre pixel Meta, mélanger leurs conversions serait incorrect en multi-tenant), token chiffré via `TokenCryptoService` (même garantie que `SocialConnection.accessToken`), saisi manuellement dans une nouvelle page `/settings/integrations` (ADMIN/OWNER) — le token CAPI n'est pas obtenable via le flux OAuth existant, généré à la main dans Meta Events Manager ; (4) `MetaCapiService.pushConversion()`, branché sur `recordManualConversion()` (best-effort, ne bloque jamais l'enregistrement de la conversion elle-même) — pousse un événement `Purchase` vers Meta avec un `fbc` dérivé du clic `fbclid` le plus récent (fenêtre de 7 jours) ; **no-op silencieux si aucune config ou aucun clic récent** (décision produit explicite : jamais d'événement non attribuable, contrairement à l'option "dégradée" écartée). Migration Prisma additive écrite à la main (`20260813210000_conversion_click_tracking`, pas de Postgres local dans cet environnement). Hors périmètre, signalé explicitement : push Google Ads, nouveau spec Playwright pour un clic réel (non simulable utilement sans scaffolding dédié), champ `eventType` sur `RecordConversionDto` (toujours `'Purchase'`), fenêtre d'attribution non configurable. Vérifié : typecheck ✅, lint ✅, build ✅ (backend + frontend, nouvelle route `/settings/integrations` compilée), **321/321 tests backend** ✅ (12 nouveaux), **90/90 tests frontend** ✅ (dont `locale-completeness.spec.ts`, 4 langues).
106. ~~Premier déploiement du dépôt sur un remote Git~~ ✅ fait — poussé sur `github.com/daviddupon88-collab/mvp-campaign-ai` (branche renommée `master` → `main` pour correspondre au déclencheur de `ci.yml`). CI GitHub Actions (4 jobs : `backend`, `frontend`, `e2e-browser`, `docker-publish`) verte de bout en bout après 3 itérations sur le job `e2e-browser` : (1) `webServer.command` de `playwright.config.ts` utilisait `next start`, incompatible avec `output: "standalone"` — remplacé par `frontend/scripts/serve-standalone.js`, qui reproduit l'étape de copie `.next/static`+`public/` déjà faite par `frontend/Dockerfile` ; (2) ce script héritait silencieusement de la variable d'environnement `PORT=3001` définie au niveau du job CI (destinée à l'étape de démarrage du backend, mais les variables de job s'appliquent à chaque étape), provoquant un `EADDRINUSE` — port codé en dur à `3000` ; (3) `frontend/e2e/i18n.spec.ts` (jamais exécuté avant ce premier run CI, Redis indisponible localement toute la session) rechargeait la page `/login` après `context.clearCookies()` sans `page.reload()` — le HTML déjà rendu en arabe (cookie `NEXT_LOCALE=ar` au moment du rendu initial) ne se met pas à jour rétroactivement, faisant échouer `getByLabel('Email')` après un timeout de 60s. `docker-publish` (bloqué depuis toujours par `needs: [backend, frontend, e2e-browser]`) tourne désormais avec succès à chaque push sur `main`.
